  ...
  return (
    <div onClick={onClick}
      ...
      <div className="flex items-start gap-3">
        <div className="w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0 bg-[#2c2c2e]">
...
function ATSPanel({ resume, onEdit }: { resume: ResumeListItem; onEdit: () => void }) {
  const [tab, setTab] = useState<'roast' | 'strengths' | 'improvements' | 'missing' | 'keywords' | 'versions'>('roast');
    ...
    <div className="space-y-4">
        ...
        <div className="flex items-start justify-between mb-5">
          ...
          <div className="flex items-center gap-2 flex-shrink-0 ml-4">
            <button onClick={onEdit}
              className="flex items-center gap-2 bg-white text-black px-4 py-2 rounded-xl text-sm font-semibold hover:bg-gray-100 transition-colors">
      ...
      <div className="bg-[#111] border border-[#222] rounded-2xl overflow-hidden">
        <div className="flex border-b border-[#222]">
          {[
            { key: 'roast', label: 'Banana Roast 🍌', icon: Flame },
          ...
          {tab === 'roast' && (
            <div className="bg-amber-950/20 border border-amber-900/30 rounded-xl p-5 space-y-3">
              <div className="flex items-center gap-2 text-amber-400 font-semibold text-xs mb-1">
                <Flame className="w-3.5 h-3.5" /> Savage Recruiter Roast
              </div>
              <p className="text-zinc-300 text-sm leading-relaxed whitespace-pre-wrap font-sans">
                {ai?.brutalRoast || "Analyzing faults... Upload or generate a resume with AI to generate a savage critique."}
              </p>
            </div>
          )}
          {tab === 'strengths' && (
